...
>
> Statut : phases 0 à 4 livrées sur la branche `migration/phase-1-pwa-indexeddb` ; Storage intégré. La bascule OpenAI et Web Push restent différées.
...
| Supabase | Installé, activé et connecté | Organisation `titou-lr` (`mjnthckdboenslxhqyyc`); projet `notion-like app` (`bmoxcrfvlwsjywtmkxvd`), région `eu-west-3`, `ACTIVE_HEALTHY`; schéma cloud et journal incrémental appliqués | Projets, SQL/Postgres, schémas, migrations, Auth, Edge Functions, Storage, branches et logs | Les 19 tables Planning ont RLS et leurs tests d'isolation passent ; la structure des 7 tables Prisma a été sauvegardée puis leur schéma distant supprimé |
| Vercel | Installé, activé et CLI connectée | Compte `titou-lr`; équipe `titou-lrs-projects` (`team_qvI695rKhMVz3kNGYGx6ziYA`); projet `planning` (`prj_FA3HpJCojZA4m4bj6ca79ZoZx9WE`) relié à GitHub et actif sur `planning-bay-six.vercel.app` | Liaison Git, builds Vite, previews et production | Exécuter la suite Playwright sur l'URL de déploiement avant toute promotion ou bascule de domaine |
