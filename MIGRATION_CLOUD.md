...
| Supabase | Installé, activé et connecté | Organisation `titou-lr` (`mjnthckdboenslxhqyyc`); projet `notion-like app` (`bmoxcrfvlwsjywtmkxvd`), région `eu-west-3`, `ACTIVE_HEALTHY`; schéma cloud, archive Prisma et journal incrémental appliqués | Projets, SQL/Postgres, schémas, migrations, Auth, Edge Functions, Storage, branches et logs | Les 19 tables Planning ont RLS et leurs tests d'isolation passent ; les 7 tables Prisma ont été archivées dans `legacy_backup_20260921` |
