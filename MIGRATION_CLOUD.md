...
| Supabase | Installé, activé et connecté | Organisation `titou-lr` (`mjnthckdboenslxhqyyc`); projet visible `notion-like app` (`bmoxcrfvlwsjywtmkxvd`), région `eu-west-3`, actuellement `INACTIVE` | Projets, SQL/Postgres, schémas, migrations, Auth, Edge Functions, Storage, branches et logs | Le rôle dev/staging/production de ce projet reste à consigner avant toute écriture distante |
