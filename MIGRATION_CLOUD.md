...
| Supabase | Installé, activé et connecté | Organisation `titou-lr` (`mjnthckdboenslxhqyyc`); projet de développement `notion-like app` (`bmoxcrfvlwsjywtmkxvd`), région `eu-west-3`, `ACTIVE_HEALTHY`; migration `initial_cloud_schema` appliquée | Projets, SQL/Postgres, schémas, migrations, Auth, Edge Functions, Storage, branches et logs | Les 19 tables Planning ont RLS et leurs tests d'isolation passent ; décider séparément du retrait ou de la sécurisation des 7 tables Prisma historiques vides |
